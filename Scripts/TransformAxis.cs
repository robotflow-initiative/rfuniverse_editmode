            ...
            if (Input.GetKeyDown(KeyCode.T))
            {
                switch (EditMain.Instance.CurrentEditMode)
                {
                    case EditMode.Move:
                        ...
                        EditMain.Instance.ChangeTransformFromAxis(controlledAttr.transform.localPosition);
                        break;
                    case EditMode.Rotate:
                        transform.eulerAngles = Vector3.zero;
                        controlledAttr.SetTransform(false, true, false, Vector3.zero, transform.eulerAngles, Vector3.one, true);
                        EditMain.Instance.ChangeTransformFromAxis(controlledAttr.transform.localEulerAngles);
                        break;
                    case EditMode.Scale:
                        transform.localScale = Vector3.one;
                        controlledAttr.SetTransform(false, false, true, Vector3.zero, Vector3.zero, transform.localScale, true);
                        EditMain.Instance.ChangeTransformFromAxis(controlledAttr.transform.localScale);
                        break;
                    default:
                        break;
                }
